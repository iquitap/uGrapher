...
int __host__ main(int argc, char* argv[]) {
    ...
    int32_t ef_feat_size=__host_input_f_size;
    ...
    vf_feat_size = __host_input_f_size;
	...
	for (int32_t trail = 0; trail < cnt; trail++) {
        ...
        {
            gpu_runtime::f_vertex_set_apply_kernel<eps_update_1><<<vf_num_cta, CTA_SIZE>>>(__host_edges__transposed, vf_feat_size);
            // cudaDeviceSynchronize();
        }
        if (run_vf) {
            cudaEventRecord(stop);
