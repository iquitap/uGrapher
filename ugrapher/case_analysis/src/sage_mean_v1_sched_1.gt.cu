...
void __device__ get_rst_21(int32_t v, int32_t f);
...
// rewrite get_rst_2
void __device__ get_rst_21(int32_t v, int32_t f) {
    feature_rst_2[f] = feature_input_fc_2[f] + feature_input_out_2[f]; //get_rst_2 not need activation
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
int __host__ main(int argc, char* argv[]) {
    ...
    int32_t ef_feat_size=__host_output_f_size;
	...
	for (int32_t trail = 0; trail < cnt; trail++) {
        ...
        if (run_ef) cudaEventRecord(start);
        {
            gpu_runtime::uGrapher_exec(0, &lb_c, &ef_num_cta, 
                __host_edges__transposed, __device_feature_input_fc_ngh_2, 0, __device_feature_input_out_2, 
                &ef_feat_size, &group_size, &par_tiling);
            // cudaDeviceSynchronize();
        }
        if (run_ef) {
        ...
        {
            gpu_runtime::f_vertex_set_apply_kernel<get_rst_21><<<vf_num_cta, CTA_SIZE>>>(__host_edges__transposed, vf_feat_size);
            // cudaDeviceSynchronize();
        }
        if (run_vf) {
            cudaEventRecord(stop);
