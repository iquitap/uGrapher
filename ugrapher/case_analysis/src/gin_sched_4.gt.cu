...
void __device__ init_feature(int32_t v, int32_t f) {
	...
	feature_rst_4[f] = ((v+1)%10 - f%f_size%10)/10000.0;
}
...
int __host__ main(int argc, char* argv[]) {
	...
	for (int32_t trail = 0; trail < cnt; trail++) {
        ...
        {
            gpu_runtime::f_vertex_set_apply_kernel<eps_update_5><<<vf_num_cta, CTA_SIZE>>>(__host_edges__transposed, vf_feat_size);
            // cudaDeviceSynchronize();
        }
        if (run_vf) {
            cudaEventRecord(stop);
