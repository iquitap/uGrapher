...
void __device__ sum_1(int32_t src, int32_t dst, int32_t feat, float* A, float* C, int Feat_Size);
void __device__ sum_1_nAtm(int32_t src, int32_t dst, int32_t feat, float* A, float* C, int Feat_Size);
...
void __device__ gpu_operator_body_0(gpu_runtime::GraphT graph, 
	...
	sum_1(dst, src, feat, A, C, Feat_Size);
}
...
void __device__ gpu_operator_body_0_nAtm(gpu_runtime::GraphT graph, 
    ...
    float* A, float* B, float* C, int Feat_Size) {
	// Body of the actual operator
	sum_1_nAtm(dst, src, feat, A, C, Feat_Size);
}
	...
	for (int32_t trail = 0; trail < cnt; trail++) {
        ...
        // gather
        {
            gpu_runtime::uGrapher_exec(0, lb_c, ef_num_cta, 
                __host_edges__transposed, __device_feature_input_fc_1, 0, __device_feature_input_out_1, 
                ef_feat_size, group_size, par_tiling);
        ...
        // gather
        {
            gpu_runtime::uGrapher_exec(1, lb_c, ef_num_cta, 
                __host_edges__transposed, __device_feature_input_fc_2, 0, __device_feature_input_out_2, 
                ef_feat_size, group_size, par_tiling);
            // cudaDeviceSynchronize();
        }
